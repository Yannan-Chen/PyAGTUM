...
class LEICAchopperlog(log.valuelogger):
    ...
    def datacollector(self):
        ...
        #EWH: adding code for updating the thickness fo cut and the feed rate depending on what is in the GUI
                                   ...
                                   if offset < self.parent.sbx_targetphase.value():
                                       if cdelta < 0.3:
                                           if cdelta  < 0.15:
                                               ...
                                               print('+ + + + IN SYNC + + + +')
                                               print("Offset: {}".format(round(offset, 3)))
                                               speed_change = abs(BaseSpeed + (0.025 * cdelta))
                                               self.parent.setTapeSpeed(speed_change)
                                               print("ATUM: tape speed up - - - - speed {}".format(round(speed_change, 4)))
...
class mainGUI(QtWidgets.QMainWindow):
    ...
    def __init__(self,uifile):
        ...
        Leica_feed = Leica.getFeedRate()
        Leica_speed = Leica.getCuttingSpeed()

        self.sbx_LeicaCutSpeed.setValue(int(Leica_speed))
        self.sbx_LeicaCutThickness.setValue(int(Leica_feed))
